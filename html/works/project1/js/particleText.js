(function( $ ) {
  $.fn.particleText = function(options) {
			...
			function Particle(ax,ay){
				...
				//=============粒子の大きさ==============
				this.r = canvas.clientWidth / 2 * 0.010;
			...
			function initScene(){
				...
				function drawOneText(){
					var hp = 2;
				    if(canvas.height <= 300 && canvas.width > 768){//変更
				    	hp = 1.5;
				    }
				...
				//ww/200とww/200はそれぞれ、X軸とY軸の粒子の間隔
...
$("#particle").particleText({
	// 表示させたいテキスト。改行の場合は<br>追加
	text: "@ta_cake_21", 
	// パーティクルの色を複数指定可能
	colors:["#000000"], 
	// slow, middle, high の3つから粒子が集まる速さを選択
	speed: "high", 
	});